...
int main(int argc, char* argv[])
{
	...
	if (argc != 4) {
		...
		printf("Example:/weather/project/tools1/bin/deletefiles /log/idc \"*.log.20*\" 0.02\n");//匹配的文件名log.20,超时时间为0.02天，“”强调参数，\为转义字符
		printf("        /weather/project/tools1/bin/deletefiles /tmp/idc/surfdata \"*.xml,*.json\" 0.01\n");
		printf("        /weather/project/tools1/bin/procctl 300  /weather/project/tools1/bin/deletefiles /log/idc \"*.log.20*\" 0.02\n"); //由调度程序启动
		printf("        /weather/project/tools1/bin/procctl 300  /weather/project/tools1/bin/deletefiles /tmp/idc/surfdata \"*.xml,*.json\" 0.01\n\n"); //由调度程序启动
		printf("这是一个工具程序，用于删除历史数据文件或日志文件。\n");
		printf("本程序把pathname目录及子目录中的与timeout匹配的machstr文件全部删除，timeout可以是小数。\n");
   ...
   */
	...
	while (true) {
		...
		if (strcmp(Dir.m_ModifyTime, strTimeOut) < 0) {
			//删除文件，调用框架中的REMOVE()函数，C语言提供的删除函数是remove()函数
			if (REMOVE(Dir.m_FullFileName) == 0) { //调用系统功能成功
				printf("REMOVE %s ok.\n", Dir.m_FullFileName);
			}
			else {
